...
class MatchTable(object):
    ...
    def match_table_optimised(self, record):
        ...
        self.logger.info("match_table_optimised Passes: {}".format(tbl._passes))
        return tbl.get_first_row().get_values()
    ...
    def drop_low_prio_rows(self):
        ...
        for col in self.cols:
            if self.len() == 1:
                ...
                self.perf.end("drop_low_prio_rows", "before {}/ after 1".format(before))
                return
            self.prune_rows(col)
        self.logger.info("before/after dropping low prio rows {}/{}".format(before, self.len()))
        self.perf.end("drop_low_prio_rows", "before: {}/ after: {}".format(before, self.len()) )
    ...
    def prune_rows(self, colname):
        ...
        * is always 1
        ...
        self.logger.info("prune rows, colname={}, row_count={}".format(colname, len(rows)))
...
class MatchRow(object):
    def __init__(self, row, cols, ordinal=1):
        ...
        self._row = {}
        self._orig_row = row
        self.logger.info("Creating Match Row {}".format(row))
...
class MatchValue(object):
    ...
    def match_value(self, str):
        if not self._pattern:
            ...
            if match:
                self.logger.debug("Match Value:'{}' to Pattern:'{}'".format(str, self._pattern))
